...
exports.index = function (url) {
    ...
    function renderImages(page, outputDir) {
        mkdirp(outputDir);
        ...
        let rasterRender = page.render(rasterPath);
         ...
         return Promise.all([pdfRender, rasterRender ]). then(function () {
    ...
    function zipFiles(inputDir, outputFileName) {
    ...
    function signFile(fileName, outputFileName) {
...
`;
    ...
    function verifySignedPackage(fileName) {
        ...
        return fsp.readFile(fileName)
            ...
            .then((zip) => {
                        return zip.file('package.zip').async('nodebuffer')
...
-----END CERTIFICATE-----
`;
                let result = verify.verify(public_key, sig, 'hex');
                console.log(result);
    ...
    let promise = loadPage(url)
        .then((page)=>loadContent(page))
        .then((page)=>renderImages(page, working_dir))
        .then((directoryPath)=>zipFiles(directoryPath, package_path))
        .then(()=>signFile(package_path, signed_path))
        .then(()=>verifySignedPackage(signed_path));
